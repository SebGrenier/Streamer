module.exports = function (grunt) {
  ...
  var ffmpegConfigShared = 
    ...
    ' --optflags="-O3" --extra-cflags="-I' + distPath + '/include -D_POSIX_C_SOURCE=200112 -D_XOPEN_SOURCE=600 -Wno-warn-absolute-paths" ' +
  ...
  var nativeRepos = {
    ...
    x264: {
      ...
      configure: 'emconfigure ./configure --disable-thread --disable-asm --disable-opencl ' +
        '--disable-cli --enable-shared --prefix=' + distPath,
      make: 'emmake make SHELL="/bin/bash -x" -j' + paraMake
    },
    /*openh264: {
  ...
  var EM_CPPFLAGS =  ' --bind -O3 -c -v -std=c++11 -Wno-warn-absolute-paths -I' + distPath + '/include ';
  ...
  var decoderSrc = ['Decoder', 'embinder'];
  var decoderTasks = [];
  var srcDir = rootPath;

  decoderSrc.forEach(function(src) {
  ...
  grunt.registerTask('help', 'help', function () {
    grunt.log.writeln('========= Decodder build script ==========');
    ...
    grunt.log.writeln('build-decoder: build wrapper codes and produce final production: decoder.js.');
    grunt.log.writeln('build: run "build-ffmpeg" and "build-decoder".');
  ...
  grunt.registerTask('build-decoder', decoderTasks);
