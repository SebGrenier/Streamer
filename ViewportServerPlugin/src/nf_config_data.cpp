...
enum {
	...
	NFCD_TYPE_ARRAY, NFCD_TYPE_OBJECT, NFCD_TYPE_UNDEFINED, NFCD_TYPE_HANDLE
};

#define NFCD_TYPE_MASK (0xf)
#define NFCD_TYPE_BITS (4)

typedef int nfcd_loc;
typedef void * (*nfcd_realloc) (void *ud, void *ptr, int osize, int nsize, const char *file, int line);
typedef void (*nfcd_handle_dealloc)(void *handle);
...
nfcd_loc nfcd_root(struct ConfigData *cd);
int nfcd_type(struct ConfigData *cd, nfcd_loc loc);
double nfcd_to_number(struct ConfigData *cd, nfcd_loc loc);
const char *nfcd_to_string(struct ConfigData *cd, nfcd_loc loc);
void *nfcd_to_handle(struct ConfigData *cd, nfcd_loc);
nfcd_handle_dealloc nfcd_to_handle_deallocator(struct ConfigData *cd, nfcd_loc loc);

int nfcd_array_size(struct ConfigData *cd, nfcd_loc arr);
nfcd_loc nfcd_array_item(struct ConfigData *cd, nfcd_loc arr, int i);

int nfcd_object_size(struct ConfigData *cd, nfcd_loc object);
nfcd_loc nfcd_object_keyloc(struct ConfigData *cd, nfcd_loc object, int i);
...
nfcd_loc nfcd_null();
nfcd_loc nfcd_undefined();
nfcd_loc nfcd_false();
nfcd_loc nfcd_true();
nfcd_loc nfcd_add_number(struct ConfigData **cd, double n);
nfcd_loc nfcd_add_string(struct ConfigData **cd, const char *s);
nfcd_loc nfcd_add_handle(struct ConfigData **cd, void *handle, nfcd_handle_dealloc deallocator);
nfcd_loc nfcd_add_array(struct ConfigData **cd, int size);
nfcd_loc nfcd_add_object(struct ConfigData **cd, int size);
void nfcd_set_root(struct ConfigData *cd, nfcd_loc root);

void nfcd_push(struct ConfigData **cd, nfcd_loc array, nfcd_loc item);
void nfcd_set(struct ConfigData **cd, nfcd_loc object, const char *key, nfcd_loc value);
void nfcd_set_loc(struct ConfigData **cd, nfcd_loc object, nfcd_loc key, nfcd_loc value);

nfcd_realloc nfcd_allocator(struct ConfigData *cd, void **user_data);
...
// Represents a stored user handle
struct handle_item
{
	void *handle;
	nfcd_handle_dealloc deallocator;
};
...
static nfcd_loc write(struct ConfigData **cdp, int type, void *p, int count, int zeroes);
static struct object_item *object_item(struct ConfigData *cd, nfcd_loc object, int i);
...
static nfcd_loc write(struct ConfigData **cdp, int type, void *p, int count, int zeroes)
{
	...
	while (cd->used_bytes + total > cd->allocated_bytes) {
		...
		cd = (ConfigData*)cd->realloc(cd->realloc_user_data, cd, cd->total_bytes, new_total_bytes,
			__FILE__, __LINE__);
...
// Frees an ConfigData object created by nfcd_make.
void nfcd_free(struct ConfigData *cd)
{
...
// Returns the handle representation of `loc`
void *nfcd_to_handle(struct ConfigData *cd, nfcd_loc loc)
{
	struct handle_item *hi = (struct handle_item *)((char *)cd + LOC_OFFSET(loc));
	return hi->handle;
}

// Returns the handle deallocator representation of `loc`
nfcd_handle_dealloc nfcd_to_handle_deallocator(struct ConfigData *cd, nfcd_loc loc)
{
	struct handle_item *hi = (struct handle_item *)((char *)cd + LOC_OFFSET(loc));
	return hi->deallocator;
}

// Returns the number of array items in `loc`.
int nfcd_array_size(struct ConfigData *cd, nfcd_loc array)
{
...
nfcd_loc nfcd_array_item(struct ConfigData *cd, nfcd_loc array, int i)
{
	assert(i >= 0);
	struct block *arr = (struct block *)((char *)cd + LOC_OFFSET(array));
...
int nfcd_object_size(struct ConfigData *cd, nfcd_loc obj)
{
...
nfcd_loc nfcd_undefined()
{
	return MAKE_LOC(NFCD_TYPE_UNDEFINED, 0);
}
...
nfcd_loc nfcd_add_number(struct ConfigData **cdp, double n)
{
...
// Adds the handle `handle` to the config data and returns its reference.
nfcd_loc nfcd_add_handle(struct ConfigData **cdp, void *handle, nfcd_handle_dealloc deallocator)
{
	struct handle_item a = {0};
	a.handle = handle;
	a.deallocator = deallocator;
	return write(cdp, NFCD_TYPE_HANDLE, &a, sizeof(a), 0);
}

// Adds a new array, preallocated to the specified size to the config data
// and returns its reference.
nfcd_loc nfcd_add_array(struct ConfigData **cdp, int allocated_size)
{
	struct block a = {0};
...
nfcd_loc nfcd_add_object(struct ConfigData **cdp, int allocated_size)
{
	struct block a = {0};
	a.allocated_size = allocated_size;
...
void nfcd_set_root(struct ConfigData *cd, nfcd_loc loc)
{
	cd->root = loc;
}

// Pushes `item` to the end of the `array`.
void nfcd_push(struct ConfigData **cdp, nfcd_loc array, nfcd_loc item)
{
...
void nfcd_set(struct ConfigData **cdp, nfcd_loc object, const char *key, nfcd_loc value)
{
...
// keys are allowed.
void nfcd_set_loc(struct ConfigData **cdp, nfcd_loc object, nfcd_loc key, nfcd_loc value)
{
...
nfcd_realloc nfcd_allocator(struct ConfigData *cd, void **user_data)
{
	*user_data = cd->realloc_user_data;
	...
	{
		struct ConfigData *cd = nfcd_make(realloc_f, 0, 0, 0);
		...
		struct ConfigData *copy = realloc_f(0, 0, 0, cd->total_bytes, __FILE__, __LINE__);
